...
**Gas Trading & Commercial Platform · v3.5 · Sprint 19**
Обновлено: 19.04.2026 · Gastrans d.o.o. Novi Sad, Serbia
Дипломный проект · Факультет технических наук · krav4ukleo@gmail.com

---

## Changelog v3.4 → v3.5 (Sprint 18–19)

### Sprint 18 (17–18.04.2026) — 22/27 SP · Closed Day 2

- **VTP Basic (NC Art.11)** — Virtual Trading Point:
  - Migration 021: `vtp_trades` table (trade_ref, direction BUY/SELL, trade_type, status).
  - 5 endpoints: `GET/POST /vtp/trades`, `GET /vtp/trades/:id`, `PATCH /vtp/trades/:id/confirm`, `GET /vtp/balance`.
  - BUY = virtual entry, SELL = virtual exit (NC Art.11 balancing perspective).
  - Seed: 6 demo trades (Газпром↔NIS, WIEH↔MET).
  - 16 integration tests.

- **Excel (xlsx) Export**:
  - `utils/xlsxExport.js` (exceljs): typed columns, auto-filter, bold headers.
  - `GET /billing/export?format=xlsx`, `/contracts/export?format=xlsx`, `/nominations/export?format=xlsx`.
  - 5 tests (header validation, numeric formatting, empty result, CSV regression).

- **FG Separate Invoice (NC Art.20.3.5)**:
  - Migration 022: `invoices.invoice_type` ∈ {CAPACITY, FUEL_GAS, IMBALANCE}.
  - `POST /billing/generate`: auto-splits FG into separate invoice when shipper has >1 active contract.
  - 3 tests (multi-product split, single-product combined, in-kind shipper).

- **OpenAPI 100% Sync** (US-1802):
  - `openapi.yaml` rewritten: 95/95 endpoints documented, 18 obsolete removed.
  - `npm run count-endpoints` → docs = actual = 0 discrepancies.

- **Load Testing** (autocannon):
  - Smoke test: avg 854 RPS, all p97.5 < 500ms, 0% errors.
  - Scripts: `npm run test:load`, `npm run test:load:stress`, `npm run test:load:spike`.
  - Report: [LOAD_TEST_RESULTS.md](./LOAD_TEST_RESULTS.md).

- **Bug Fixes**:
  - `nextReference()` duplicate key → rewritten COUNT→MAX.
  - `allocated_kwh_h` column doesn't exist → removed from nominations.js + analytics.js.
  - 8 pre-existing test failures fixed.

### Sprint 19 (18–19.04.2026)

- **VTP Balance Integration** (NC Art.12.3 + Art.11):
  - `GET /balance` rewritten: nominations + VTP trades combined view.
  - Balance = (nom_entry + vtp_buy) - (nom_exit + vtp_sell).
  - `balanced` flag: |diff| < 1 kWh/h.

- **Transparency Portal (NC Art.24)** — 4 public endpoints, NO JWT:
  - `GET /public/capacity` — technical vs contracted per IP (anonymized).
  - `GET /public/auctions` — upcoming + recent auction calendar.
  - `GET /public/gas-quality` — latest gas quality per IP (NC Art.17).
  - `GET /public/fuel-gas-price` — current FG tender price (NC Art.18.5.1.4).
  - Rate-limited: 30 req/min per IP. 6 tests.

- **Diploma Artifacts Index**: [DIPLOMA_ARTIFACTS_INDEX.md](./DIPLOMA_ARTIFACTS_INDEX.md) — полный индекс 60+ артефактов проекта.

### Migrations: 000–022 (добавлены 021–022 в Sprint 18)
### Endpoints: 99 authoritative · OpenAPI 99/99 (100% sync)
### Tests: 565 passed (37 suites)

---

## Changelog v3.3 → v3.4 (Sprint 15–17)

### Sprint 15 (03.04.2026)
- NC consistency audit: исправлены 3 расхождения IP-кодов в demo data. NC Coverage 79%.

### Sprint 16 (06–10.04.2026) — rescoped
- **Migration 017**: `capacity_kwh_h` as native column (замена runtime `MWh/d × 1000 / 24`).
- **Migration 018**: `oba_daily_imbalances` — OBA Settlement NC Art.15 (TSO-to-TSO only, read-only).
- **NC Art.12.3 binding rule**: shippers always balanced (`nominated = allocated = matched`). US-1603 (Shipper Imbalance Charge) отменён навсегда, замещён US-1603b OBA.
- UI cleanup: billing tariff SQL, contracts mapping, credit NC Art.5.1.6 (ОСВОБОЖДЁН для рейтинговых), gas quality columns.

### Sprint 17 (13–24.04.2026) — ~29/33 SP · Session 15.04.2026
- **FG Art.18 Hotfix** (US-1708..1713, ADR-018/019):
  - Binding rule: `FG > 0 ⟺ flow_direction ∈ {KIREVO_HORGOS, KIREVO_HORGOS_AND_SERBIA} AND election='CASH' AND AAQ_horgos > 0`.
  - Обоснование: Art.19.1.4 — GMS-4 Gospođinci без preheater; компрессор только для транзита.
  - Migration 019: `shippers.fuel_gas_election` ∈ {CASH, IN_KIND}.
  - Data fix: 4 invoice NIS исправлены, коррекция 299 535,64 EUR. Отчёт: [FG_DATA_FIX_REPORT.md](./FG_DATA_FIX_REPORT.md), сессия: [SESSION_2026-04-15_FG_HOTFIX.md](./SESSION_2026-04-15_FG_HOTFIX.md).
- **NC Art.13 Completion** (US-1703/1704):
  - Migration 020: `adjacent_tso_matches`.
  - `adjacentTsoService.js`: Lesser Rule (OURS/THEIRS/EQUAL) + mock FGSZ/Bulgartransgaz/TRANSPORTGAS SRBIJA.
  - Endpoints: `POST /nominations/:id/match-adjacent`, `GET /nominations/:id/matching-result`.
  - Новый статус nomination: `MATCHED_ADJACENT`.
  - NC Art.13 coverage: 67% → **100%** ✅.
- **Analytics Dashboard** (US-1705): `GET /analytics/volumes|revenue|utilization` с group-by month/point/shipper.
- **CSV Export** (US-1706): `utils/csvExport.js` (BOM, RFC 4180) + `GET /billing/export|/contracts/export|/nominations/export`.
- **Sprint 16 Test Debt** (US-1701): +30 tests (OBA 11 + capacity 8 + shipper balance 11).
- **Endpoint Audit** (US-1702): actual = **82 endpoints** (ранее 99 в docs). `npm run count-endpoints`. OpenAPI sync → Sprint 18 (US-1802).
- **Регресс**: Jest 442 → **527 passed** (+85 тестов), 8 pre-existing failures unchanged.

### Migrations (Sprint 16-17): 017–020
### Endpoints (Sprint 17): 82 authoritative (OpenAPI sync achieved Sprint 18)
...
| Покрытие NC compliance | ~40% | **79%** (Sprint 12 baseline) + Art.13=100% (Sprint 17) + Art.18 correctness fix (Sprint 17) |
| Тесты | — | **527/535** passing (Sprint 17 Day 3 · +85 от Sprint 16 baseline) |
| API endpoints | — | **82** (authoritative, `npm run count-endpoints` · Sprint 17 DEBT-02) |
...
| База данных | PostgreSQL | 15–17 | Хранение данных, миграции 000–015 |
| Frontend | Vanilla JS + HTML5 | Sprint 15 | SPA-интерфейс (GTCP_MVP.html), RBP Bridge UI (4 вкладки) |
| API Client | api.js v2.1 | 82 endpoints (audited 15.04.2026) | Все модули + RBP Bridge, Reserve Prices, Invoice Line Items |
...
Текущий результат (Sprint 15, 03.04.2026):
...
### 9.6 Auction Calendar — Day-Centric View (Sprint 14, NC Art.7 + CAM NC)

Sprint 14 — day-centric auction calendar (CAM NC MAR0277-24). 115 DB auctions (Yearly + Quarterly + Monthly + CR) + Daily/Within-Day generated on-the-fly per Gas Day. Yearly Firm = only surrendered LT capacity (NC Art.7.1.2).

**API Endpoints (Sprint 14):**

| Метод | URL | Описание |
|---|---|---|
| GET | `/api/v1/auctions/calendar/grid` | Product × Month grid for Gas Year (Y/Q/M status per month) |
| GET | `/api/v1/auctions/calendar/days?year=YYYY&month=M` | Day-centric calendar: DB auctions (Y/Q/M) + on-the-fly Daily/WD per day |
| GET | `/api/v1/capacity/available` | Real-time Available Capacity (NC Art.7.1.1 + Art.7.3) — SQL on every request |

**Available Capacity formula (NC Art.7.1.1):**

```
Firm ST Available = Technical[IP] − SUM(active bookings) + SUM(surrendered)
CR Available (Art.7.3) = SUM(contracted in physical direction) − SUM(CR already contracted)
Yearly Firm Available (Art.7.1.2) = SUM(surrendered LT) — 0 if no surrender
```
...
### 14.3 Fuel Gas (NC Art.18.2.1 + Art.18.3 + Art.19.1.4)

**Правило начисления (14.04.2026):**
...
Для всех остальных маршрутов (`KIREVO_EXIT_SERBIA`, 4× Commercial Reverse) → **FG_fee = 0**.

Обоснование: компрессорная станция (Art.19.1.4.3) нужна только для транзита до Horgoš; GMS-4 Gospođinci, через который выходит домашний поток NIS, **не имеет preheating unit** (Art.19.1.4 упоминает только GMS-2 и GMS-3) → ни CS, ни PHG не аллоцируется.

**Формула (применяется только к транзитным shipper'ам с election=CASH):**

```
FG_kwh = X1 × Q_horgos − KN         (для KIREVO_HORGOS и транзитной доли KIREVO_HORGOS_AND_SERBIA)
FG_eur = (FG_kwh / 1000) × price_eur_mwh
```

`Q_serbia` обнуляется на уровне billing — domestic exit освобождён от FG.
...
### Сводная таблица (82 authoritative endpoints · audited Sprint 17 DEBT-02)

| Модуль | Базовый путь | Изменения Sprint 15–17 |
|---|---|---|
| Auth | `/api/v1/auth` | |
| Shippers | `/api/v1/shippers` | `fuel_gas_election` column (migration 019) |
| Contracts | `/api/v1/contracts` | `GET /export` CSV (Sprint 17) |
| Billing | `/api/v1/billing` | FG guards (Art.18 binding), `GET /export` CSV (Sprint 17) |
| Credits | `/api/v1/credits` | NC Art.5.1.6 ОСВОБОЖДЁН UI |
| Auctions | `/api/v1/auctions` | |
| Capacity | `/api/v1/capacity` | `capacity_kwh_h` native (migration 017) |
| **Nominations** | `/api/v1/nominations` | `GET /export` CSV, **`POST /:id/match-adjacent`**, **`GET /:id/matching-result`** (Sprint 17) |
| Balance | `/api/v1/balance` | `GET /oba/daily|monthly/:month|summary` (Sprint 16) |
| Audit | `/api/v1/audit` | |
| System Params | `/api/v1/system-params` | |
| RBP Bridge | `/api/v1/rbp` | |
| **Analytics (new)** | `/api/v1/analytics` | **`GET /volumes|revenue|utilization`** (Sprint 17 US-1705) |
| Health | `/api/v1/health` | |
| **Всего** | | **82 actual** (`npm run count-endpoints`) |

**OpenAPI coverage:** 58/82 (71%). Полная синхронизация → Sprint 18 US-1802.

### Новые endpoints Sprint 17 (детально)

| Endpoint | Назначение | NC Reference |
|---|---|---|
| `POST /nominations/:id/match-adjacent` | Lesser Rule matching с adjacent TSO (mock). Body: `{scenario: 'less'|'more'|'equal'}` | NC Art.13 |
| `GET /nominations/:id/matching-result` | Сводка по nomination + adjacent matches + summary | NC Art.13 |
| `GET /analytics/volumes` | Monthly nominated/matched/allocated kWh per IP | NC Art.12 |
| `GET /analytics/revenue` | Monthly capacity+FG+total revenue per shipper | NC Art.20 |
| `GET /analytics/utilization` | Tech vs contracted kWh/h per IP + utilization_pct | NC Art.7 + AERS |
| `GET /billing/export` | CSV invoice register (BOM + RFC 4180) | NC Art.20 |
| `GET /contracts/export` | CSV contracts register | NC Art.6 |
| `GET /nominations/export` | CSV nominations register | NC Art.12 |
...
### 16.2 NC Compliance Matrix (82%, Sprint 17)
...
| Art.11 VTP | 1 | 0 | 0 | 1 | 0% (Sprint 18 US-1803) |
| Art.12 Nominations | 8 | 8 | 0 | 0 | 100% |
| Art.13 Matching | 3 | **3** | 0 | 0 | **100%** ✅ (Sprint 17 US-1703/1704) |
| Art.14 Restrictions | 2 | 0 | 0 | 2 | 0% |
| Art.15 Balancing | 3 | 2 | 1 | 0 | 83% (Sprint 16 OBA read-only) |
| Art.17 Gas Quality | 2 | 1 | 0 | 1 | 50% |
| Art.18 Fuel Gas | 3 | **3** | 0 | 0 | 100% + correctness fix (Sprint 17 Art.19.1.4 binding) |
| Art.19 Metering | 2 | 2 | 0 | 0 | 100% (GMS-2/3/4 + compressor) |
| Art.20 Billing | 5 | 5 | 0 | 0 | 100% |
| Art.24 Transparency | 1 | 0 | 1 | 0 | 50% |
| **ИТОГО** | **72** | **59** | **3** | **10** | **82%** (Sprint 17) |
...
| **14** | **31.03.2026** | **35** | **✅** | **Auction Calendar (CAM NC MAR0277-24, 115 auctions), Available Capacity Engine (real-time SQL), EDIGAS v5.1 NOMINT, Credit Instruments, Data Integrity, UAT fixes (10)** |
| **15** | **03.04.2026** | **16** | **✅** | **NC Consistency: demo data NC-compliant (20/20 IP codes), documentation alignment v3.3, NC full read verification (111 pages)** |

**Кумулятив на 03.04.2026: ~552 SP · 442/442 тестов · NC 79% (55/70) · 96 endpoints · Migrations 000–015**
...
| Database | PostgreSQL | 15–17 | Storage, migrations 000–015 |
| Frontend | Vanilla JS + HTML5 | Sprint 15 | SPA interface + RBP Bridge UI (4 tabs), real data from API |
| API Client | api.js v2.1 | 82 endpoints (audited 15.04.2026) | All modules + RBP Bridge, Reserve Prices, Invoice Line Items |
...
### 6.4 Fuel Gas (NC Art.18.2.1 + Art.18.3 + Art.19.1.4)

**Allocation rule (updated 14.04.2026):**
...
All other routes (KIREVO_EXIT_SERBIA, 4× Commercial Reverse) → FG_fee = 0.
```

Rationale: compressor station (Art.19.1.4.3) is needed only for transit to Horgoš; GMS-4 Gospođinci (domestic exit used by NIS) has **no preheating unit** (Art.19.1.4 lists only GMS-2 and GMS-3). Neither CS nor PHG is allocated to domestic/CR shippers.

**Formula (applied only to transit shippers with election=CASH):**

```
FG_kwh = X1 × Q_horgos − KN       (Q_serbia zeroed in billing layer)
FG_eur = (FG_kwh / 1000) × price_eur_mwh
```

Where: X1 = 0.42% (compressors), KN = D-2 correction.
...
### Summary Table (82 authoritative endpoints, audited Sprint 17 DEBT-02)

| Module | Base Path | Sprint 15–17 Changes |
|---|---|---|
| Auth | `/api/v1/auth` | |
| Shippers | `/api/v1/shippers` | `fuel_gas_election` column (migration 019) |
| Contracts | `/api/v1/contracts` | `GET /export` CSV (Sprint 17) |
...
| Auctions | `/api/v1/auctions` | |
| Capacity | `/api/v1/capacity` | `capacity_kwh_h` native (migration 017) |
| **Nominations** | `/api/v1/nominations` | `GET /export`, **`POST /:id/match-adjacent`**, **`GET /:id/matching-result`** (Sprint 17) |
| Balance | `/api/v1/balance` | `GET /oba/daily|monthly/:month|summary` (Sprint 16) |
| Audit | `/api/v1/audit` | |
| System Params | `/api/v1/system-params` | |
| RBP Bridge | `/api/v1/rbp` | |
| **Analytics (new)** | `/api/v1/analytics` | **`GET /volumes|revenue|utilization`** (Sprint 17 US-1705) |
| Health | `/api/v1/health` | |
| **Total** | | **82 actual** (`npm run count-endpoints`) |

**OpenAPI coverage:** 58/82 (71%). Full sync → Sprint 18 US-1802.

### New Endpoints Sprint 17 (detailed)

| Endpoint | Purpose | NC Reference |
|---|---|---|
| `POST /nominations/:id/match-adjacent` | Lesser Rule matching with adjacent TSO (mock) | NC Art.13 |
| `GET /nominations/:id/matching-result` | Double-Sided matching summary with adjacent matches | NC Art.13 |
| `GET /analytics/volumes` | Monthly nominated/matched/allocated kWh per IP | NC Art.12 |
| `GET /analytics/revenue` | Monthly capacity + FG + total revenue per shipper | NC Art.20 |
| `GET /analytics/utilization` | Tech vs contracted kWh/h per IP + utilization_pct | NC Art.7 + AERS |
| `GET /billing/export` | CSV invoice register (BOM + RFC 4180) | NC Art.20 |
...
### 16.2 NC Compliance Matrix (79%, Sprint 15)
...
| Art.15 Balancing | 3 | 2 | 1 | 0 | 83% (Sprint 16 OBA read-only) |
| Art.17 Gas Quality | 2 | 1 | 0 | 1 | 50% |
| Art.18 Fuel Gas | 3 | **3** | 0 | 0 | 100% + correctness fix (Sprint 17: Art.19.1.4 binding rule) |
| Art.19 Metering | 2 | 2 | 0 | 0 | 100% (GMS-2/3/4 + compressor) |
| Art.20 Billing | 5 | 5 | 0 | 0 | 100% |
| Art.24 Transparency | 1 | 0 | 1 | 0 | 50% |
| **TOTAL** | **72** | **59** | **3** | **10** | **82%** (Sprint 17) |
...
**Cumulative 03.04.2026: ~552 SP · 442/442 tests · NC 79% (55/70) · 96 endpoints · Migrations 000–015**
